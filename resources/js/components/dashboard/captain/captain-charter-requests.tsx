import { Link } from '@inertiajs/react';
import { CalendarDays, Clock3 } from 'lucide-react';
import { requests } from '@/routes';
import { captainRequests } from './captain-dashboard-data';

export function CaptainCharterRequests() {
    return (
        <section className="rounded-2xl border border-[#d4dbe3] bg-white p-4 shadow-sm">
            <h3 className="text-[17px] font-semibold text-[#0f172a]">
                Charter Requests
            </h3>
            <p className="text-[13px] text-[#6b7280]">Pending availability requests</p>

            <div className="mt-4 space-y-4">
                {captainRequests.map((item) => (
                    <article
                        key={`${item.yachtName}-${item.date}`}
                        className="rounded-xl border border-[#e5e7eb] bg-white p-4 transition-colors hover:border-[#d4dbe3]"
                    >
                        <div className="mb-3 flex items-start justify-between gap-4">
                            <div>
                                <p className="text-[15px] font-semibold text-[#111827]">
                                    {item.yachtName}
                                </p>
                                <p className="text-[12.5px] text-[#6b7280]">
                                    {item.yachtSpec}
                                </p>
                            </div>
                            <span className="rounded-full bg-[#6b7280] px-2.5 py-0.5 text-[10px] font-semibold tracking-wide text-white uppercase">
                                {item.status}
                            </span>
                        </div>

                        <div className="mb-4 space-y-2">
                            <p className="flex items-center gap-2 text-[13px] text-[#4b5563]">
                                <CalendarDays className="h-4 w-4 text-[#9ca3af]" />
                                {item.date}
                            </p>
                            <p className="flex items-center gap-2 text-[13px] text-[#4b5563]">
                                <Clock3 className="h-4 w-4 text-[#9ca3af]" />
                                {item.duration}
                            </p>
                        </div>

                        <div className="flex gap-3">
                            <button
                                type="button"
                                className="inline-flex items-center rounded-lg bg-[#0D314D] px-5 py-2 text-[12px] font-medium text-white shadow-sm transition-colors hover:bg-[#0a273f]"
                            >
                                Accept
                            </button>
                            <button
                                type="button"
                                className="inline-flex items-center rounded-lg border border-[#e5e7eb] bg-white px-5 py-2 text-[12px] font-medium text-[#374151] shadow-sm transition-colors hover:bg-[#f9fafb]"
                            >
                                Decline
                            </button>
                        </div>
                    </article>
                ))}
            </div>

            <Link
                href={requests()}
                className="mt-5 inline-flex h-12 w-full items-center justify-center rounded-lg bg-[#0D314D] text-[15px] font-medium text-[#FFFFFF] transition-colors hover:bg-[#0a273f]"
            >
                View All Requests
            </Link>
        </section>
    );
}
