...
export function RequestCard({
    ...
    const deckhandIsPending =
        hasSelectedDeckhand &&
        request.deckhandInfo?.selectedDeckhand?.responseStatus !== 'available';
    const deckhandAccepted =
        hasSelectedDeckhand &&
        request.deckhandInfo?.selectedDeckhand?.responseStatus === 'available';
    const selectedByMe =
        request.deckhandInfo?.selectedDeckhand?.selectedByMe === true;
    const deckhandHireFullySigned =
        request.deckhandInfo?.deckhandHireFullySigned === true;

    // Agreement for THIS captain (only present when selectedByMe)
    const deckhandAgreement = request.agreements?.find(
        (a) => a.type === 'deckhand_hire',
    );
    ...
    const acceptDisabled =
        ...
        (crewRole === 'captain' &&
            ...
            !(
                request.deckhandInfo?.selectedDeckhand?.responseStatus ===
                    'available' &&
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
                    <div className="mx-4 w-full max-w-md rounded-lg bg-white p-6 shadow-xl">
                        ...
                        <p className="mb-6 text-sm text-gray-600">
                            Are you sure you want to sign the Deckhand Hire
                            Agreement? This will generate the PDF and finalize
                            the process.
                        </p>
                        <div className="flex justify-end gap-3">
                            ...
                            <button
                                ...
                                onClick={() => {
                                    ...
                                    router.post(
                                        `/requests/${request.id}/sign-deckhand-agreement`,
                                        {},
                                        {
                                            preserveScroll: true,
                                            onSuccess: (page) => {
                            ...
                            <div className="min-w-0 flex-1">
                                <a
                                    ...
                                    onClick={(e) => {
                                        e.preventDefault();
                                        router.visit(
                                            `/captains/${request?.captainInfo?.id}`,
                                        );
                                    }}
                                >
                ...
                */}
                {isPending && crewRole === 'captain' && deckhandIsPending && (
                    <p className="rounded-md border border-yellow-200 bg-yellow-50 px-4 py-2 text-[13px] text-yellow-700">
                        Your deckhand invitation is pending. You can accept this
                        charter once the deckhand confirms.
                    </p>
                )}
                            ...
                            <button
                                ...
                                onClick={() => {
                                    if (!hasSelectedDeckhand) {
                                        onSelectDeckhand?.(
                                            request.id,
                                            request.charterEventId as string,
                                        );
                                    }
                                }}
                                title={
                                    deckhandIsPending
                                        ? 'Deckhand invitation already sent'
                                        : 'Select a deckhand for this charter'
                                }
                            ...
                            <>
                                {deckhandAgreement?.downloadUrl ? (
                                    <a
                                        ...
                                        className="hidden cursor-pointer items-center justify-center gap-2 rounded-md bg-[#14532d] px-5 py-2.5 text-[13px] font-medium text-white shadow-sm transition-colors hover:bg-[#166534]"
                                    >
