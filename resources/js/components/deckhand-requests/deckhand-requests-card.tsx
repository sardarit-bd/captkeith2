...
const statusStyles: Record<DeckhandInterestRecord['status'], string> = {
    pending: 'bg-[#fef9c3] text-[#854d0e]',
    accepted: 'bg-[#dcfce7] text-[#166534]',
    declined: 'bg-[#fee2e2] text-[#991b1b]',
};

const statusLabels: Record<DeckhandInterestRecord['status'], string> = {
    pending: 'Pending',
    accepted: 'Accepted',
    declined: 'Declined',
};

export function DeckhandRequestCard({
    interest,
}: {
    interest: DeckhandInterestRecord;
}) {
    ...
    function handleRespond(status: 'accepted' | 'declined') {
        ...
        router.patch(
            `/deckhand-requests/${interest.id}/respond`,
            { status },
            {
                preserveScroll: true,
        ...
        <article className="rounded-2xl border border-[#d4dbe3] bg-white p-6 shadow-sm transition-shadow hover:shadow-md">
            <div className="mb-5 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                <div className="flex items-center gap-4">
                    {interest.deckhandPhoto ? (
                        <img
                            src={interest.deckhandPhoto}
                            alt={interest.deckhandName}
                    ...
                    <div>
                        <h3
                            ...
                            onClick={() =>
                                router.get(`/deckhands/${interest.deckhandId}`)
                            }
                        >
                            {interest.deckhandName}
                        </h3>
            ...
            <div className="mb-5 flex flex-wrap gap-2">
                {[
                    interest.hasServerExperience,
                    interest.hasBartendingExperience,
