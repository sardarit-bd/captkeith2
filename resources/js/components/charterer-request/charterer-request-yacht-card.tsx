...
import { charterRequestYacht } from './charterer-request-data';

export function ChartererRequestYachtCard() {
    return (
        <section className="rounded-2xl border border-[#edf2f7] bg-white p-6 shadow-sm sm:p-8">
            ...
            <div className="flex flex-col items-start gap-6 md:flex-row lg:gap-8">
                <img
                    src={charterRequestYacht.image}
                    alt={charterRequestYacht.name}
                ...
                <div className="flex-1 space-y-6">
                    <div>
                        <h4 className="text-2xl leading-tight font-bold text-[#111827]">
                            {charterRequestYacht.name}
                        </h4>
                        <p className="mt-1 text-sm text-[#6b7280]">
                            {charterRequestYacht.registrationNumber}
                        </p>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <DetailsItem
                            label="Type"
                            value={charterRequestYacht.type}
                        />
                        <DetailsItem
                            label="Length"
                            value={charterRequestYacht.length}
                        />
                    </div>
                    ...
                    <div className="space-y-3 pt-2">
                        <p className="flex items-center gap-2.5 text-sm text-[#4b5563]">
                            ...
                            <span>{charterRequestYacht.marina}</span>
                        </p>
                        <p className="flex items-center gap-2.5 text-sm text-[#4b5563]">
                            <Compass className="h-4 w-4 text-[#9ca3af]" />
                            <span>
                                Operating Area:{' '}
                                {charterRequestYacht.operatingArea}
                            </span>
                        </p>
                    </div>
                </div>
