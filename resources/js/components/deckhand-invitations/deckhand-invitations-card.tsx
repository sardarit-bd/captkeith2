...
const statusStyles: Record<DeckhandInvitationRecord['status'], string> = {
    pending: 'bg-[#fef9c3] text-[#854d0e]',
    accepted: 'bg-[#dcfce7] text-[#166534]',
    declined: 'bg-[#fee2e2] text-[#991b1b]',
};

const statusLabels: Record<DeckhandInvitationRecord['status'], string> = {
    pending: 'Pending',
    accepted: 'Accepted',
    declined: 'Declined',
};

export function DeckhandInvitationCard({
    invitation,
}: {
    invitation: DeckhandInvitationRecord;
}) {
    ...
    function handleRespond(status: 'accepted' | 'declined') {
        ...
        router.patch(
            `/deckhand-invitations/${invitation.id}/respond`,
            { status },
            {
                preserveScroll: true,
        ...
        <article className="rounded-2xl border border-[#d4dbe3] bg-white p-6 shadow-sm transition-shadow hover:shadow-md">
            <div className="mb-5 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                ...
                <div className="flex shrink-0 items-center gap-2">
                    ...
                    <span
                        className={`rounded-full px-3 py-1 text-[11px] font-semibold tracking-wide uppercase ${statusStyles[invitation.status]}`}
                    >
            ...
            <div className="mb-5 flex items-center gap-3 rounded-xl border border-[#eef2f6] bg-[#f8fafc] p-4">
                {invitation.vesselImage ? (
                    <img
                        src={invitation.vesselImage}
                        alt={invitation.vesselName}
                ...
                <div>
                    <p className="text-[13px] font-semibold text-[#111827]">
                        {invitation.vesselName}
                    </p>
                    <p className="text-[12px] text-[#6b7280]">
                        {invitation.vesselSpec}
                    </p>
