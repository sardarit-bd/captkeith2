...
export function YachtsMatchGrid() {
    ...
    // console.log(vessels);
    console.log(auth?.user?.captain_profile?.is_verified);
    if (auth?.user?.captain_profile?.is_verified === false) {
        return (
            <div className="flex flex-col items-center justify-center py-20 text-center">
                ...
                <Link
                    href={myProfile()}
                    className="inline-flex items-center rounded-lg bg-[#35ADD5]/70 px-6 py-2.5 text-[14px] font-medium text-white shadow-sm transition-colors hover:bg-[#35ADD5]"
                >
                    Set Up Profile
                </Link>
                <Link
    ...
    } else if(auth?.user?.captain_profile?.is_verified === "pending"){
       <div className="flex flex-col items-center justify-center py-20 text-center">
                <span className="mb-4 inline-flex h-16 w-16 items-center justify-center rounded-full bg-[#EFF6FF]">
                    <Ship className="h-8 w-8 text-[#2563eb]" />
                </span>
                <h3 className="mb-2 text-[18px] font-semibold text-[#0f172a]">
                    Did you complete your profile? Request Again
                </h3>
                <p className="mb-6 max-w-sm text-[14px] text-[#6b7280]">
                    We match vessels to your license type, endorsement, tonnage
                    rating, and years of experience. Get admin approvals for your profile to see your matches.
                </p>
                <Link
                    href={myProfile()}
                    className="inline-flex items-center rounded-lg bg-[#35ADD5] px-6 py-2.5 text-[14px] font-medium text-white shadow-sm transition-colors hover:bg-[#35ADD5]"
